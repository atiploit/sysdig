...
constexpr const int64_t CGROUP_VAL_MAX = 1UL << 42u;
...
bool get_cgroup_resource_limits(const delayed_cgroup_key& key, delayed_cgroup_value& value)
{
	...
	g_logger.format(sinsp_logger::SEV_DEBUG, "(async-cg) Got cgroup limits for container [%s]: mem_limit=%ld, cpu_shares=%ld cpu_quota=%ld cpu_period=%ld",
...
void delayed_cgroup_lookup::tick(sinsp_container_manager* manager)
{
	...
	for(const auto& it : cgroup_data)
	{
		...
		if(container)
		{
			g_logger.format(sinsp_logger::SEV_DEBUG, "(async-cg) Storing limits for container [%s]: mem_limit=%ld, cpu_shares=%ld, cpu_quota=%ld, cpu_period=%ld",
					it.first.m_container_id.c_str(),
					it.second.m_memory_limit, it.second.m_cpu_shares, it.second.m_cpu_quota, it.second.m_cpu_period);
		...
		{
			g_logger.format(sinsp_logger::SEV_NOTICE, "(async-cg) Dropping limits for already gone container [%s]: mem_limit=%ld, cpu_shares=%ld, cpu_quota=%ld, cpu_period=%ld",
					it.first.m_container_id.c_str(),
					it.second.m_memory_limit, it.second.m_cpu_shares, it.second.m_cpu_quota, it.second.m_cpu_period);
		}
	}
